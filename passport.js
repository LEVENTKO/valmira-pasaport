import{doc as p,getDoc as v}from"https://www.gstatic.com/firebasejs/12.19.0/firebase-firestore.js";import{initializeApp as b}from"https://www.gstatic.com/firebasejs/12.19.0/firebase-app.js";import{getFirestore as f,connectFirestoreEmulator as w}from"https://www.gstatic.com/firebasejs/12.19.0/firebase-firestore.js";var c={apiKey:"AIzaSyBKyMh_iLw8aIoj3jlrjJOIoN_0BDh0078",authDomain:"valmira-pasaport.firebaseapp.com",projectId:"valmira-pasaport",storageBucket:"valmira-pasaport.firebasestorage.app",messagingSenderId:"439318009946",appId:"1:439318009946:web:f10ebd01195d366bc0e063"};var z=b(c),d=f(z),$=["localhost","127.0.0.1"].includes(location.hostname);$&&w(d,location.hostname,8080);var i=r=>String(r??"").replace(/[&<>"']/g,l=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[l]),n=(r,l=2)=>Number(r||0).toLocaleString("tr-TR",{minimumFractionDigits:l,maximumFractionDigits:l});var u={collection:"Denizden a\u011F \xE7\u0131karma (DYKD)",transport:"Liman \u2192 geri d\xF6n\xFC\u015F\xFCm tesisi nakliye",granulation:"Gran\xFCl \xFCretimi (ZetaPlast)",water:"Y\u0131kama suyu",manufacturing:"\xC7er\xE7eve imalat\u0131 (Billur Optik)",packaging:"Ambalaj",distribution:"Da\u011F\u0131t\u0131m"};var y=document.getElementById("passport");function m(){document.title="Pasaport bulunamad\u0131 \xB7 Valmira",y.innerHTML=`<main><div class="wrap"><div class="card login"><h1>Pasaport bulunamad\u0131</h1>
    <p class="muted" style="margin-bottom:0">Bu QR koduna ait bir Valmira pasaportu bulunamad\u0131. Kodu yeniden okutmay\u0131 deneyin.</p></div></div></main>`}async function S(){let r=new URLSearchParams(location.search).get("s");if(!r)return m();let l=await v(p(d,"items",r)).catch(()=>null);if(!l?.exists())return m();let _=await v(p(d,"batches",l.data().batchId));if(!_.exists())return m();let e=_.data(),a=e.result,g=e.photos||[];document.title=`${e.model_name} \xB7 Valmira Karbon Pasaportu`;let h=a?.ok?Math.max(...Object.values(a.stages)):0,k=a?.ok?Math.max(a.frame_kg,a.virgin_frame_kg):1,s=(t,o=0)=>n(t*1e3,o);y.innerHTML=`
<header class="hero"><div class="wrap">
  <div class="tag">Valmira \xB7 Dijital \xDCr\xFCn Pasaportu</div>
  <h1>${i(e.model_name)}</h1>
  <div>${[e.color,"Hayalet a\u011Fdan \xFCretilmi\u015F g\xFCne\u015F g\xF6zl\xFC\u011F\xFC"].filter(Boolean).map(i).join(" \xB7 ")}</div>
  <div class="serial" style="margin-top:8px">Seri no: ${i(r)} \xB7 Parti ${i(e.code)}</div>
</div></header>
<main style="padding-top:0"><div class="wrap">
${a?.ok?`<section class="stats lift">
  <div class="stat"><div class="l">Denizden \xE7\u0131kar\u0131lan hayalet a\u011F</div><div class="v">${n(a.ghost_net_dry_g,0)} <small>g</small></div><div class="l">kuru \xB7 denizden ilk \xE7\u0131kt\u0131\u011F\u0131nda ${n(a.ghost_net_wet_g,0)} g</div></div>
  <div class="stat"><div class="l">Temizlenen a\u011F alan\u0131</div><div class="v">${n(a.seabed_area_m2*1e4,0)} <small>cm\xB2</small></div></div>
  <div class="stat"><div class="l">\xC7er\xE7evenin karbon ayak izi</div><div class="v">${s(a.frame_kg)} <small>g CO2e</small></div></div>
  <div class="stat"><div class="l">Virgin plastik \xE7er\xE7eveye g\xF6re</div><div class="v">%${n(a.frame_saving_pct,0)} <small>daha az</small></div></div>
</section>`:""}
${e.story?`<section class="card" style="margin-top:16px"><h2>Hik\xE2yesi</h2><p style="margin:0;white-space:pre-line">${i(e.story)}</p></section>`:""}
${g.length?`<section class="card"><h2>Denizden g\xF6zl\xFC\u011Fe</h2><div class="gallery">
  ${g.map(t=>`<figure><img src="${i(t.url)}" alt="${i(t.caption||e.model_name)}" loading="lazy">${t.caption?`<figcaption>${i(t.caption)}</figcaption>`:""}</figure>`).join("")}
</div></section>`:""}
<section class="card"><h2>Yolculu\u011Fu</h2>
  ...
  <dl style="margin-top:16px">
    ${e.collection_region?`<dt>A\u011Flar\u0131n topland\u0131\u011F\u0131 yer</dt><dd>${i(e.collection_region)}</dd>`:""}
    ${e.collection_period?`<dt>Toplama d\xF6nemi</dt><dd>${i(e.collection_period)}</dd>`:""}
    ${e.production_date?`<dt>\xDCretim tarihi</dt><dd>${i(new Date(e.production_date).toLocaleDateString("tr-TR",{year:"numeric",month:"long"}))}</dd>`:""}
...
${a?.ok?`<section class="card"><h2>Karbon ayak izi nereden geliyor?</h2>
  <p style="margin-top:0">G\xF6zl\xFC\u011F\xFCn tamam\u0131, camlar dahil <strong>${n(a.total_min_kg,2)}\u2013${n(a.total_max_kg,2)} kg CO2e</strong>. \xC7er\xE7eve k\u0131sm\u0131 \xF6l\xE7\xFClm\xFC\u015F \xFCretim verisiyle hesapland\u0131; camlar i\xE7in tedarik\xE7inin ortalama aral\u0131\u011F\u0131 kullan\u0131ld\u0131.</p>
  ...
  <div class="compare" style="margin-top:18px">
    <div><div class="lab"><span>Bu \xE7er\xE7eve (hayalet a\u011Fdan)</span><strong>${s(a.frame_kg)} g CO2e</strong></div><div class="track"><div class="fill" style="width:${a.frame_kg/k*100}%;background:var(--sea-2)"></div></div></div>
    <div><div class="lab"><span>Ayn\u0131 \xE7er\xE7eve, virgin plastikle</span><strong>${s(a.virgin_frame_kg)} g CO2e</strong></div><div class="track"><div class="fill" style="width:${a.virgin_frame_kg/k*100}%;background:#9fb3c0"></div></div></div>
  </div>
  <p class="small muted" style="margin-bottom:0">Yaln\u0131zca hammaddeye bak\u0131ld\u0131\u011F\u0131nda hayalet a\u011F gran\xFCl\xFC, virgin PA6'ya g\xF6re %${n(a.raw_saving_pct,0)} daha d\xFC\u015F\xFCk emisyonludur (aral\u0131k %${n(a.raw_saving_pct_low,0)}\u2013${n(a.raw_saving_pct_high,0)}).</p>
</section>`:""}
